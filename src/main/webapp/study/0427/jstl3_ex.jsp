...
<div class="container">
  ...
  <form>
    ...
    <div>
	    ...
	    <input type="submit" value="3번째문자 검색" class="btn btn-info ml-2"/>
    </div>
    <hr/>
    ...
    <c:if test="${!empty content && !empty searchString}">
      <c:if test="${fn:indexOf(content,'o') ne -1}"> <!-- 값이 있으면 아래 수행 -->
	      <c:set var="position" value="${fn:indexOf(content,'o')}"/> <!-- var position = ${fn:indexOf(content,'o')} -->
